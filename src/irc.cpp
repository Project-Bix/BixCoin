...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #bixcoincoinTEST\r");
            Send(hSocket, "WHO #bixcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #bixcoincoin\n");
            // randomly join #bixcoincoin00-#bixcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bixcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bixcoincoin%02d\r", channel_number).c_str());
        };
