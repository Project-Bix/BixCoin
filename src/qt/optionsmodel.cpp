...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBixCoinAmount"))
        settings.setValue("nAnonymizeBixCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeBixCoinAmount = settings.value("nAnonymizeBixCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeBixCoinAmount"))
        SoftSetArg("-anonymizeBixCoinamount", settings.value("nAnonymizeBixCoinAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeBixCoinAmount:
            nAnonymizeBixCoinAmount = value.toInt();
            settings.setValue("nAnonymizeBixCoinAmount", nAnonymizeBixCoinAmount);
            emit anonymizeBixCoinAmountChanged(nAnonymizeBixCoinAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
