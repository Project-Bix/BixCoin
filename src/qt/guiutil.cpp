...
{
    // Convert bixcoin:// to bixcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bixcoin://"))
    {
        uri.replace(0, 12, "bixcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BixCoin.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IBixCoinLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IBixCoinLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "bixcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BixCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bixcoin-qt [" + tr("command-line options") + "]                     " + "\n";
