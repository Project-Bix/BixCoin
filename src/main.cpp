...
int nCoinbaseMaturity = 79; // 80 in total
...
const string strMessageMagic = "BixCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
        nSubsidy = 15000 * COIN; // For sale @ 0.00008btc per 1 BiX
    }
    else if (pindexBest->nHeight < 11)
    {
        nSubsidy = 1 * COIN; // coin sale sending fee
    }
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  // 52.78% yearly interest 
    if (pindexBest->nHeight < 150)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); // 52.78% yearly interest
    }
    else if (pindexBest->nHeight < 15000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10; // 527.8% yearly interest
    }
    else if (pindexBest->nHeight < 30000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 5;  // 263.9% yearly interest
    }
    else if (pindexBest->nHeight < 45000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2.5; // 131.95% yearly interest
    }
    else if (pindexBest->nHeight < 60000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1.25;  // 65.97% yearly interest
    }
    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = 15 * 60;  // 15 mins
