...
int nAnonymizeBixCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BixCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BixCoin
    // Mac: ~/Library/Application Support/BixCoin
    // Unix: ~/.BixCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BixCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bixcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bixcoind.pid"));
